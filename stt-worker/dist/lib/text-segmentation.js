...
exports.splitIntoSentencesKorean = splitIntoSentencesKorean;
const FILLER_LINE_RE = /^(음+|어+|그+|막+|아+|어음+|음음+|저기|그냥|네+|예+|음\.\.\.|어\.\.\.)[.!?…~]*$/;
...
function splitParagraphToSentences(paragraph) {
    ...
    if (!p)
        return [];
    const regex = /(?<=[.!?…]|다\.|요\.|죠\.|함\.|임\.|니다\.|까요\?|겠죠\?|했죠\.|해요\.)\s+/g;
    const tokens = p
        .split(regex)
        .map((s) => s.trim())
        .filter((s) => s.length > 2);
...
function splitIntoSentencesKorean(text) {
    if (!text?.trim())
        return [];
    const lines = text
        .replace(/\r\n/g, "\n")
