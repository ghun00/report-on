...
exports.rewriteSectionToReportTone = rewriteSectionToReportTone;
const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
const OPENAI_BASE_URL = process.env.OPENAI_BASE_URL ?? "https://api.openai.com/v1";
const OPENAI_MODEL_REWRITE = process.env.OPENAI_MODEL_REWRITE ?? process.env.OPENAI_MODEL ?? "gpt-4o-mini";
const OPENAI_TEMPERATURE_REWRITE = Number.parseFloat(process.env.OPENAI_TEMPERATURE_REWRITE ?? "0.2");
...
[SECTION_TEXT]
{{SECTION_TEXT}}`;
function buildRewriteUserPrompt(sectionText, enforceLength = false) {
...
async function callRewriteLLM(sectionText, enforceLength = false) {
    if (!OPENAI_API_KEY) {
...
async function rewriteSectionToReportTone(sectionText, opts) {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        ...
        try {
            ...
            console.warn("[section-rewriter] attempt failed:", "attempt=", attempt, "reason=", e instanceof Error ? e.message : String(e));
        }
    }
