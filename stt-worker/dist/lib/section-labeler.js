...
function buildChunkPrompt(sentences) {
...
function validateSections(sections, sentenceCount) {
...
async function requestChunkSections(chunkSentences) {
    if (!OPENAI_API_KEY)
    ...
    const reqBody = {
        ...
        messages: [
            {
                role: "system",
                content: "당신은 상담 대본 문장 배열의 섹션 라벨러다. 문장을 재작성/요약하지 말고 섹션 경계와 제목만 반환한다.",
            },
...
function fallbackSections(sentenceCount) {
    if (sentenceCount <= 0)
        return [];
    if (sentenceCount < 12)
