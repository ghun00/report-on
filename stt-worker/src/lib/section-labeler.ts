...
const TITLE_BANNED_RE =
  /(전반|후반|파트\s*\d+|part\s*\d+|상담\s*내용|기타|종합|일반|섹션\s*\d+)/i;
const TITLE_KEYWORD_RE =
  /(내신|성적|모의고사|최저|생기부|전공|지원|전략|리스크|액션|대학|카드|수능|목표|진단|합격|불합격|상향|하향)/;

function getMinSectionsByTranscriptLength(transcriptLen: number): number {
  if (transcriptLen >= 7000) return 10;
  if (transcriptLen >= 4000) return 8;
  if (transcriptLen >= 2000) return 6;
  return 3;
}

function calculateTranscriptLength(sentences: string[]): number {
...
function buildChunkPrompt(
  sentences: string[],
  minSectionsForChunk: number,
  retryReason?: string
): string {
  const sentencesJson = JSON.stringify(
    sentences.map((s, i) => ({ index: i, text: s })),
    null,
    2
  );
...
function validateSections(
  ...
  for (const s of sorted) {
    ...
    if (TITLE_BANNED_RE.test(title)) return `generic title not allowed: ${title}`;
    if (!TITLE_KEYWORD_RE.test(title)) return `title missing keyword: ${title}`;
...
async function requestChunkSections(
    ...
    messages: [
      {
        role: "system",
        content:
          "당신은 '상담 transcript 문장 목록'을 주제 전환 기준으로 챕터링하는 라벨러다. 역할은 오직 섹션 경계(start/end)와 제목(title)을 정하는 것이다. 문장 재작성/요약/편집/해석은 금지한다.",
      },
  ...
  const parsed = JSON.parse(raw) as { sections?: LabeledSection[] };
  if (!Array.isArray(parsed.sections)) throw new Error("invalid sections payload");
  return parsed.sections;
}

function inferKeywordTitle(slice: string[]): string {
  const text = slice.join(" ").slice(0, 2000);
  if (/내신|성적|등급/.test(text)) return "내신/성적 진단 및 상향 전략";
  if (/모의고사|최저|수능/.test(text)) return "모의고사/최저 기준 대응 전략";
  if (/생기부|학생부|활동/.test(text)) return "생기부 강점·약점 점검";
  if (/전공|학과|변경/.test(text)) return "전공 변경 논의와 리스크";
  if (/대학|지원|카드|원서/.test(text)) return "지원 대학/카드 조합 전략";
  if (/리스크|위험|불리|부담/.test(text)) return "핵심 리스크와 대응 포인트";
  if (/계획|실행|액션|다음/.test(text)) return "다음 액션 및 실행 계획";
  return "지원전략 및 리스크 점검";
}

function fallbackSections(sentences: string[], minSections: number): LabeledSection[] {
...
function minSectionsForChunk(
  totalMinSections: number,
...
export async function labelSectionsWithLLM(sentences: string[]): Promise<{
  ...
  for (const chunk of chunks) {
    ...
    const chunkMinSections = minSectionsForChunk(
      minSections,
      sentences.length,
    ...
    for (let attempt = 1; attempt <= MAX_LABEL_ATTEMPTS; attempt++) {
      try {
        const candidate = await requestChunkSections(
          chunkSentences,
          chunkMinSections,
          attempt > 1 ? `섹션 수가 너무 적음. 주제 전환마다 더 잘게 나누라. 이전 사유: ${lastReason}` : undefined
        );
  ...
  if (merged.length < minSections && sentences.length > 0) {
    console.warn(
      "[section-labeler] merged sections too few, applying global fallback:",
      "sectionCount=",
      merged.length,
      "minSections=",
      minSections
