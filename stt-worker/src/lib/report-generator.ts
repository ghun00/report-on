...
function extractDetailedTextAndSectionCount(obj: unknown): {
  detailedText: string;
  detailedSectionsCount: number | null;
} {
...
function validateCoverageAndLength(transcript: string, parsed: unknown): CoverageValidation {
...
async function callOpenAI(transcript: string, extraUserInstruction = ""): Promise<unknown> {
...
export async function generateReportJson(transcript: string): Promise<ReportJsonSchema> {
  ...
  let lastCoverage: CoverageValidation | null = null;
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!validateReportJson(parsed)) {
        ...
        console.warn(
          "[report-generator] attempt validation failed:",
          "attempt=",
          attempt,
        ...
        console.log(
          "[report-generator] attempt metrics:",
          "attempt=",
          attempt,
          "transcriptLen=",
        ...
        console.warn(
          ...
          attempt,
          "reason=",
          coverage.failureReason ?? "coverage validation failed"
        );
      }
    } catch (e) {
